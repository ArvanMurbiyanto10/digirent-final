        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 md:p-8 text-gray-900 dark:text-gray-100">
                    ...
                    {{-- Form menunjuk ke route 'update' dan menggunakan method 'PUT' --}}
                    <form action="{{ route('admin.news-items.update', $newsItem) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="title" id="title" value="{{ old('title', $newsItem->title) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label for="link_url" class="block text-sm font-medium">URL Tautan Berita (Link Eksternal)</label>
                                <input type="url" name="link_url" id="link_url" value="{{ old('link_url', $newsItem->link_url) }}" required placeholder="https://contohberita.com/..." class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label for="description" class="block text-sm font-medium">Deskripsi Singkat</label>
                                <textarea name="description" id="description" rows="3" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $newsItem->description) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                @if ($newsItem->image_path)
                                    <div class="mt-4">
                                        ...
                                        <img src="{{ Storage::url($newsItem->image_path) }}" alt="Gambar lama" class="h-20 w-auto rounded-md">
                                    </div>
                                @endif
                            </div>
