        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div class="mb-8 max-w-lg mx-auto">
                <div class="relative">
                    ...
                    <input type="text" id="search-input" placeholder="Cari produk (misal: iPhone 15 Pro)..."
                        class="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-full shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition bg-white text-gray-900">
                </div>
            </div>
            ...
            <div id="product-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                @forelse ($products as $product)
                    <div class="product-card bg-white rounded-xl shadow-md overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:-translate-y-2"
                        ...
                        <div class="p-5 flex flex-col">
                            <h3
                                class="text-lg font-bold text-gray-800 truncate group-hover:text-purple-600 transition-colors duration-300">
                                {{ $product->name }}</h3>
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100">
                                <span class="text-2xl font-bold text-gray-900">Rp
                                    {{ number_format($product->price_per_day) }}</span>
                                <span class="text-sm text-gray-500">/ hari</span>
                            </div>
                            <a href="{{ route('products.show', $product) }}"
                                class="flex items-center justify-center w-full bg-purple-600 text-white py-2.5 rounded-lg mt-4 font-semibold hover:bg-purple-700 transition-all duration-300">Lihat
                                Detail</a>
                        </div>
                    </div>
                @empty
    ...
    @push('styles')
        <style>
            ...
            .filter-btn:hover {
                color: #7e22ce;
                border-color: #d8b4fe;
                /* Warna border ungu muda saat hover */
